...
private let HTTP_HEADERS: [String: String] = [
...
private func espnNewsURL(sport: String, league: String, limit: Int) -> URL {
    URL(string: "https://site.api.espn.com/apis/site/v2/sports/\(sport)/\(league)/news?limit=\(limit)")!
}

func fetchLeagueNews(leagueKey: String, limit: Int = 20) async -> [NewsItem] {
    guard let league = LEAGUES.first(where: { $0.key == leagueKey }) else { return [] }
    let url = espnNewsURL(sport: league.sport, league: league.league, limit: limit)
    guard let data = try? await httpGet(url: url),
...
func fetchAllSportsNews(limit: Int = 10) async -> [NewsItem] {
...
private func espnArticleToNewsItem(_ a: [String: Any], leagueKey: String, leagueLabel: String) -> NewsItem {
    let id = a["id"].map { "espn-\(leagueKey)-\($0)" } ?? "espn-\(leagueKey)-\(Int.random(in: 0..<Int.max))"
...
func fetchPlayerNews(playerName: String, teamName: String = "", limit: Int = 5) async -> [NewsItem] {
...
private func parseGoogleNewsRSS(xml: String, limit: Int, source: NewsItem.NewsSource) -> [NewsItem] {
    var items: [NewsItem] = []
...
func httpGet(url: URL) async throws -> Data {
    var request = URLRequest(url: url)
    for (k, v) in HTTP_HEADERS { request.setValue(v, forHTTPHeaderField: k) }
